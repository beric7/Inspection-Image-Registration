...
def homography(match_obj, save_dir):
    ...
    save_path = save_dir + 'homography_warped_image/' + match_obj.get_sample_image_name() +'homography_warped.png'
    ...
    av.save(save_dir + 'exemplar_av_image/' + match_obj.get_sample_image_name() + '_average_homography.png')
    
    # keypoint output
    cv2.imwrite(save_dir + 'keypoints/'+ match_obj.get_sample_image_name() + '_keypoints.png', out)

def ransac_flow(ransac_network, coarse_model, match_obj, save_dir):
    
    # initialize
    target_image_path = match_obj.get_target_image_path()
    ...
    # coarse alignment
    sample_coarse, sample_coarse_im, flow_coarse, grid, featt = coarse_alignment(ransac_network, coarse_model, sample_image, target_image)
    
    # resize image
    target_image = target_image.resize((sample_coarse_im.size[0], sample_coarse_im.size[1]))
    
    # save coarse alignment image
    sample_coarse_im.save(save_dir+'coarse_warped_image/'  + match_obj.get_sample_image_name() + '_coarse_warped.png')
        
    target_image.save(save_dir + '/resize_target/target.png')
    
    # average image
    av = get_Avg_Image(sample_coarse_im, target_image)
    av.save(save_dir + 'exemplar_av_image/' + match_obj.get_sample_image_name() + '_average_coarse.png')

def fusion(ransac_network, coarse_model, match_obj, save_dir):
    
    # initialize
    target_image_path = match_obj.get_target_image_path()
    target_image = Image.open(target_image_path).convert('RGB')
    
    homography_warped_image_path = save_dir + 'homography_warped_image/' + match_obj.get_sample_image_name() +'homography_warped.png'
    sample_image_path = homography_warped_image_path
    sample_image_path = match_obj.get_sample_image_path()
    sample_image = Image.open(sample_image_path).convert('RGB')
    ...
    # save coarse alignment image
    sample_coarse_im.save(save_dir+'fusion_warped_image/'  + match_obj.get_sample_image_name() + '_fusion_warped.png')
    
    # average image
    av = get_Avg_Image(sample_coarse_im, target_image)
    av.save(save_dir + 'exemplar_av_image/' + match_obj.get_sample_image_name() + '_average_fusion.png')
...
def find_peak(model, keypts_target, keypts_sample, target_image_path, sample_image_list, 
              bool_count, past_count, sample_image_number, mconf_smallest, best_match_dict_list, best_image_number):
    ...
    if bool_count and len(best_match_dict_list) != 0:
        return best_match_dict_list, best_image_number
    else:
...
def find_matching_image(target_image_dir, sample_image_dir, save_dir, model, 
    ...
    for target_n in tqdm(range(0, len(target_image_list))):
        ...
        best_match_dict_list, best_image_number = find_peak(model, keypts_target, keypts_sample, target_image_path, 
                                                            sample_image_list, bool_count, past_count,
                                                            sample_image_number, mconf_smallest, best_match_dict_list, best_image_number)
        ...
        if not os.path.exists(save_dir + '/frame_{}'.format(target_n)+'/coarse_warped_image/'): # if it doesn't exist already
            os.makedirs(save_dir + '/frame_{}'.format(target_n)+'/coarse_warped_image/')
            
        # --------------------------------------------------------------------
        # FUSION
        # --------------------------------------------------------------------        
        if not os.path.exists(save_dir + '/frame_{}'.format(target_n)+'/fusion_warped_image/'): # if it doesn't exist already
            os.makedirs(save_dir + '/frame_{}'.format(target_n)+'/fusion_warped_image/')
        ...
        for match_dict in best_match_dict_list:
            if count_exemplar < exemplar_image_num:
                ...
                ransac_flow(ransac_network, coarse_model, match_obj, save_dir + '/frame_{}'.format(target_n)+'/')
                fusion(ransac_network, coarse_model, match_obj, save_dir + '/frame_{}'.format(target_n)+'/')
                # exemplar image
                cv2.imwrite(save_dir + '/frame_{}'.format(target_n)+'/' + 'exemplar_sample_image/' + match_obj.get_sample_image_name() + '.png' , match_obj.get_sample_image())
                count_exemplar+=1
            else:
                break
