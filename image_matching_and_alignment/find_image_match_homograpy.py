...
def homography(mkpts0, mkpts1, sample_image, save_dir):
    ...
    cv2.imwrite(save_dir+'warped.png', homography_warp)
...
def find_matching_image(target_image_dir, sample_image_dir, save_dir, model, 
    ...
    for target_n in tqdm(range(0, len(target_image_list))):
        ...
        if not os.path.exists(save_dir + '/frame_{}'.format(target_n)+'/warped/'): # if it doesn't exist already
            os.makedirs(save_dir + '/frame_{}'.format(target_n)+'/warped/')
            
        homography(keypts_target, keypts_sample, sample_image, save_dir + '/frame_{}'.format(target_n)+'/warped/')
        # create a file called frame_{}
