# -*- coding: utf-8 -*-
"""
Created on Wed Jul 28 11:23:10 2021
...
from ransac_flow_alignment import ransac_flow_coarse, ransac_flow_fine
# ----------------------------------------------------------------------------
...
def fine_alignment_two_img(target_image_path, sample_image_path, save_dir, model, coarse_model, ransac_network):

    target_image = cv2.imread(target_image_path)
    target_image_name = os.path.splitext(os.path.basename(target_image_path))[0]

    iden = save_dir.split('/')[-2]
    if not os.path.exists(save_dir + '/fine_warped_image_'+iden+'/'): # if it doesn't exist already
        os.makedirs(save_dir + '/fine_warped_image_'+iden+'/')
    ...
    fine_im = ransac_flow_fine(ransac_network, coarse_model, target_image, sample_image, 
                                   match_obj.get_sample_image_name(), save_dir)
    
    fine_im.save(save_dir + '/fine_warped_image_'+iden+'/' + match_obj.get_sample_image_name() + '.jpg')
