...
from superGlue_model.utils import (compute_pose_error, compute_epipolar_error,
                          ...
                          scale_intrinsics)
from load_superGlue_model import load_superglue_model
# ============================================================================

from model_class import model, get_matches
...
def coarse_alignment_two_img(target_image_path, sample_image_path, save_dir, model, 
                             coarse_model, ransac_network):

    target_image = cv2.imread(target_image_path)
    target_image_name = os.path.splitext(os.path.basename(target_image_path))[0]

    if not os.path.exists(save_dir + '/warped/'): # if it doesn't exist already
        os.makedirs(save_dir + '/warped/')
    ...
    if not os.path.exists(save_dir +'/exemplar_sample_image/'): # if it doesn't exist already
        os.makedirs(save_dir + '/exemplar_sample_image/')
        
    if not os.path.exists(save_dir + '/exemplar_warped_image/'): # if it doesn't exist already
        os.makedirs(save_dir + '/exemplar_warped_image/') 
        
    if not os.path.exists(save_dir +'/exemplar_av_image/'): # if it doesn't exist already
        os.makedirs(save_dir + '/exemplar_av_image/') 
