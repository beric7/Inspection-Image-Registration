...
source_target_image_dir = base + 'beam_2_targets_average/complete_split/'
save_target_image_dir = base + 'beam_2_targets_average_l1_predictions_split/'
source_frames_image_dir = base + id

count = 0
for frame_name in tqdm(os.listdir(source_frames_image_dir)):
    frame_dir = source_target_image_dir + frame_name + '/'
    for image_name in tqdm(os.listdir(frame_dir)):
        print(image_name)
        image_path = frame_dir + image_name
        destination_mask = save_target_image_dir + frame_name + '/predicted_target_mask/'
        destination_overlays = save_target_image_dir + frame_name + '/combined_overlays/'
        destination_ohev = save_target_image_dir + frame_name + '/ohev/'
        destination_fit_im = save_target_image_dir + frame_name + '/fit_image/'
...
# HOMOGRAPHY
for frame_name in tqdm(os.listdir(source_frames_image_dir)):
    frame_dir = source_frames_image_dir + frame_name + '/homography_warped_image/'
    for image_name in os.listdir(frame_dir):
        image_path = frame_dir + image_name
        destination_mask = source_frames_image_dir + frame_name + '/homography_predicted_sample_masks/'
        destination_overlays = source_frames_image_dir + frame_name + '/homography_combined_overlays/'
        destination_ohev = source_frames_image_dir + frame_name + '/homography_ohev/'
        destination_fit_im = source_frames_image_dir + frame_name + '/homography_fit_pred_image/'
        generate_images(model, image_path, image_name, destination_mask, destination_overlays, destination_ohev, destination_fit_im)   
