...
def make_matrix_from_list_jpeg(image_dir, source_image_list):
    ...
    for image_name in tqdm(source_image_list):
        image_name = image_name.split('.')[-2]+'.png'
...
def save_concat_image(destination_dir, image_name, image):
    ...
    cv2.imwrite(destination_dir+image_name+'_concat.png', image)
...
id = 'beam_2_targets_many_frames_predictions_cut/'
image_name = 'v2'
source_image_dir = base + id
destination_dir = base + '/concatinated_samples/' + id
image_list_dir = base + 'beam_2_targets_many_frames_cut/'

image_list = make_image_list(image_list_dir)
directory_list = [source_image_dir]

concat_lists(image_list, directory_list, base +'/multi_concat/', pad=True, orientation=1)

dim1 = 1
dim2 = 7

concat_obj = concat( base +'/multi_concat/', destination_dir, image_list_dir, image_name, dim1, dim2)
